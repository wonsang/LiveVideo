...
void write_out_picture(StorablePicture *p, int p_out)
{
  ...
  if(rgb_output)
  {
	...
	//img2buf (p->imgUV[1], rand_pic[picture_num][2], p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
	//Added-End
  ...
  //Added by Wonsang You at FEB 7, 2007
  //img2buf (p->imgY, rand_pic[picture_num][0], p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
  //Added-End
  ...
  if (p->chroma_format_idc!=YUV400)
  {
	...
	//Added by Wonsang You at FEB 7, 2007
	//img2buf (p->imgUV[0], rand_pic[picture_num][1], p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
	//Added-End
  ...
  else
  {
    if (input->write_uv)
    {
      ...
      //Added by Wonsang You at Oct-13-2006
	  //img2buf (p->imgUV[0], dec_seq[picture_num][1], p->size_x/2, p->size_y/2, symbol_size_in_bytes, crop_left/2, crop_right/2, crop_top/2, crop_bottom/2);
	  //Added-End
