  ...
  int img_block_y)      //!< location of block Y, multiples of 4
{
  ...
  // form predictor pels
  //ws 071015
  if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
  {
	  if (block_available_up)
	  {
	    //(pix_b.pos_x, pix_b.pos_y)의 매크로블록이 배경 영역이라면,
	    mbx = (int)(pix_b.pos_x/16);
		mby = (int)(pix_b.pos_y/16);
		if(blk_dec_enb[mby*mb_width+mbx]==1)
		{
		...
		{
			P_A = BkdBuf[0][(pix_b.pos_y)*iwidth + (pix_b.pos_x+0)];
			P_B = BkdBuf[0][(pix_b.pos_y)*iwidth + (pix_b.pos_x+1)];
			P_C = BkdBuf[0][(pix_b.pos_y)*iwidth + (pix_b.pos_x+2)];
			P_D = BkdBuf[0][(pix_b.pos_y)*iwidth + (pix_b.pos_x+3)];
		}
	  }
	  else
	  {
	  ...
	  {
	    mbx = (int)(pix_c.pos_x/16);
		mby = (int)(pix_c.pos_y/16);
		if(blk_dec_enb[mby*mb_width+mbx]==1)
		{
		...
		{
			P_E = BkdBuf[0][(pix_c.pos_y)*iwidth + (pix_c.pos_x+0)];
			P_F = BkdBuf[0][(pix_c.pos_y)*iwidth + (pix_c.pos_x+1)];
			P_G = BkdBuf[0][(pix_c.pos_y)*iwidth + (pix_c.pos_x+2)];
			P_H = BkdBuf[0][(pix_c.pos_y)*iwidth + (pix_c.pos_x+3)];
		}
	  }
	  else
	  {
	  ...
	  {
	    mbx = (int)(pix_a[0].pos_x/16);
		mby = (int)(pix_a[0].pos_y/16);
		if(blk_dec_enb[mby*mb_width+mbx]==1)
		{
		...
		{
			P_I = BkdBuf[0][(pix_a[0].pos_y)*iwidth + (pix_a[0].pos_x)];
			P_J = BkdBuf[0][(pix_a[1].pos_y)*iwidth + (pix_a[1].pos_x)];
			P_K = BkdBuf[0][(pix_a[2].pos_y)*iwidth + (pix_a[2].pos_x)];
			P_L = BkdBuf[0][(pix_a[3].pos_y)*iwidth + (pix_a[3].pos_x)];
		}
	  }
	  else
	  {
	  ...
	  {
		mbx = (int)(pix_d.pos_x/16);
		mby = (int)(pix_d.pos_y/16);
		if(blk_dec_enb[mby*mb_width+mbx]==1)
		{
		...
		{
			P_X = BkdBuf[0][(pix_d.pos_y)*iwidth + (pix_d.pos_x)];
		}
	  }
	  else
	  {
      ...
      for(i=0;i<BLOCK_SIZE;i++)
	  {
	    if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
		{
			...
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				...
				img->mpr[i+ioff][j+joff]=BkdBuf[0][(pix_b.pos_y)*iwidth + (pix_b.pos_x+i)];/* store predicted 4x4 block */
		}
		else
		{
			img->mpr[i+ioff][j+joff]=imgY[pix_b.pos_y][pix_b.pos_x+i];/* store predicted 4x4 block */
  ...
  case HOR_PRED:                        /* horizontal prediction from left block */
    ...
    for(j=0;j<BLOCK_SIZE;j++)
      for(i=0;i<BLOCK_SIZE;i++)
	  {
	    if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
		{
			mbx = (int)(pix_a[j].pos_x/16);
			mby = (int)(pix_a[j].pos_y/16);
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				img->mpr[i+ioff][j+joff]=imgY[pix_a[j].pos_y][pix_a[j].pos_x]; /* store predicted 4x4 block */
			else
				img->mpr[i+ioff][j+joff]=BkdBuf[0][(pix_a[j].pos_y)*iwidth + (pix_a[j].pos_x)]; /* store predicted 4x4 block */
		}
		else
		{
			img->mpr[i+ioff][j+joff]=imgY[pix_a[j].pos_y][pix_a[j].pos_x]; /* store predicted 4x4 block */
                         ...
                         int predmode)        //!< prediction mode
{
    ...
    for(j=0;j<MB_BLOCK_SIZE;j++)
      for(i=0;i<MB_BLOCK_SIZE;i++)
	  {
	    ...
	    if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
		{
			...
			{
				img->mpr[i][j]=BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+i)];// store predicted 16x16 block
			}
		}
		else
			img->mpr[i][j]=imgY[up.pos_y][up.pos_x+i];// store predicted 16x16 block
  ...
  case HOR_PRED_16:                        // horisontal prediction from left block
    ...
    for(j=0;j<MB_BLOCK_SIZE;j++)
      for(i=0;i<MB_BLOCK_SIZE;i++)
	  {
	    ...
	    if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
		{
			mbx = (int)(left[j+1].pos_x/16);
			mby = (int)(left[j+1].pos_y/16);
			if(blk_dec_enb[mby*mb_width+mbx]==1)
			{
			...
			{
				img->mpr[i][j]=BkdBuf[0][(left[j+1].pos_y)*iwidth + (left[j+1].pos_x)]; // store predicted 16x16 block
			}
		}
		else
			img->mpr[i][j]=imgY[left[j+1].pos_y][left[j+1].pos_x]; // store predicted 16x16 block
	  ...
	  if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
	  {
		  if (up_avail)
		  {
			  ...
			  if(blk_dec_enb[mby*mb_width+mbx]==1)
					...
					s1 += BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+i)];    // sum hor pix
		  }

		  if (left_avail)
		  {
			  mbx = (int)(left[i+1].pos_x/16);
			  mby = (int)(left[i+1].pos_y/16);
			  if(blk_dec_enb[mby*mb_width+mbx]==1)
					s2 += imgY[left[i+1].pos_y][left[i+1].pos_x];    // sum vert pix
			  else
					s2 += BkdBuf[0][(left[i+1].pos_y)*iwidth + (left[i+1].pos_x)];    // sum vert pix
		  }
	  }
	  else
	  {
  ...
  case PLANE_16:// 16 bit integer plan pred
    ...
    for (i=1;i<9;i++)
    {
	  ...
	  if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
	  {
		  if (i<8)
		  {
			...
			{
				ih += i*(BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+7+i)] - BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+7-i)]);
			}
		  }
		  else
		  {
		    ...
		    mby = (int)(up.pos_y/16);			
		    if(blk_dec_enb[mby*mb_width+mbx]==1)
				a = imgY[up.pos_y][up.pos_x+7+i];
			else
				a = BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+7+i)];

			mbx = (int)(left[0].pos_x/16);
		    mby = (int)(left[0].pos_y/16);			
		    if(blk_dec_enb[mby*mb_width+mbx]==1)
				b = imgY[left[0].pos_y][left[0].pos_x];
			else
				b = BkdBuf[0][(left[0].pos_y)*iwidth + (left[0].pos_x)];
		  ...
		  if(blk_dec_enb[mby*mb_width+mbx]==1)
			  ...
			  iv += i*(BkdBuf[0][(left[8+i].pos_y)*iwidth + (left[8+i].pos_x)] - BkdBuf[0][(left[8-i].pos_y)*iwidth + (left[8-i].pos_x)]);
	  }
	  else
	  {
		  if (i<8)
	...
	if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
	{
	    ...
	    mby = (int)(up.pos_y/16);			
	    if(blk_dec_enb[mby*mb_width+mbx]==1)
			a = imgY[up.pos_y][up.pos_x+15];
		else
			a = BkdBuf[0][(up.pos_y)*iwidth + (up.pos_x+15)];

		mbx = (int)(left[16].pos_x/16);
	    mby = (int)(left[16].pos_y/16);			
	    if(blk_dec_enb[mby*mb_width+mbx]==1)
			b = imgY[left[16].pos_y][left[16].pos_x];
		else
			b = BkdBuf[0][(left[16].pos_y)*iwidth + (left[16].pos_x)];
...
void intrapred_chroma(struct img_par *img, int uv)
{
  ...
  int mbx,mby;
  int mb_width = (int)(iwidth/16);
  unsigned short ta1,ta2,ta3,tb1,tb2,tb3,tb4;
  //end-ws 071017
    ...
    for(b8=0; b8<img->num_blk8x8_uv/2;b8++)
    {
      for (b4=0; b4<4; b4++)
      {
		...
		if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
		{
			...
			{
			case 0:  //===== TOP LEFT =====
			  if      (up_avail)       for (i=blk_x;i<(blk_x+4);i++)  
			  {
				  mbx = (int)(up.pos_x/16);
				  mby = (int)(up.pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
					  s0 += imgUV[uv][up.pos_y][up.pos_x + i];
				  else
						s0 += BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x + i)>>1)];
			  }
			  if      (left_avail[0])  for (i=blk_y;i<(blk_y+4);i++)  
			  {
				  mbx = (int)(left[i].pos_x/16);
				  mby = (int)(left[i].pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
					  s2 += imgUV[uv][left[i].pos_y][left[i].pos_x];
				  else
						s2 += BkdBuf[uv+1][(left[i].pos_y>>1)*iwidth + (left[i].pos_x>>1)];
			  }
			...
			case 1: //===== TOP RIGHT =====
			  if      (up_avail)       for (i=blk_x;i<(blk_x+4);i++)  
			  {
				  ...
				  mby = (int)(up.pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
					  s1 += imgUV[uv][up.pos_y][up.pos_x + i];
				  else
						s1 += BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x + i)>>1)];
			  }
			  else if (left_avail[0])  for (i=blk_y;i<(blk_y+4);i++)  
			  {
				  mbx = (int)(left[i].pos_x/16);
				  mby = (int)(left[i].pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
			  ...
			  break;
			case 2: //===== BOTTOM LEFT =====
			  if      (left_avail[1])  for (i=blk_y;i<(blk_y+4);i++)  
			  {
				  mbx = (int)(left[i].pos_x/16);
				  mby = (int)(left[i].pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
					  s3 += imgUV[uv][left[i].pos_y][left[i].pos_x];
				  else
						s3 += BkdBuf[uv+1][(left[i].pos_y>>1)*iwidth + (left[i].pos_x>>1)];
			  }
			  else if (up_avail)       for (i=blk_x;i<(blk_x+4);i++)  
			  {
				  mbx = (int)(up.pos_x/16);
				  mby = (int)(up.pos_y/16);			
				  if(blk_dec_enb[mby*mb_width+mbx]==1)
  ...
  if (PLANE_8 == currMB->c_ipred_mode)
  {
	...
	if((tracking_flag==0)&&(segmentation_flag==1)&&(bkd_ref_flag==1))
	{
	    ...
	    mby = (int)(up.pos_y/16);			
	    if(blk_dec_enb[mby*mb_width+mbx]==1)
			ta1 = imgUV[uv][up.pos_y][up.pos_x+cr_MB_x-1];
		else
			ta1 = BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x+cr_MB_x-1)>>1)];

		mbx = (int)(left[0].pos_x/16);
	    mby = (int)(left[0].pos_y/16);			
	    if(blk_dec_enb[mby*mb_width+mbx]==1)
			tb1 = imgUV[uv][left[0].pos_y][left[0].pos_x];
		else
			tb1 = BkdBuf[uv+1][(left[0].pos_y>>1)*iwidth + (left[0].pos_x>>1)];

		mbx = (int)(left[cr_MB_y].pos_x/16);
	    mby = (int)(left[cr_MB_y].pos_y/16);			
	    if(blk_dec_enb[mby*mb_width+mbx]==1)
			tb2 = imgUV[uv][left[cr_MB_y].pos_y][left[cr_MB_y].pos_x];
		else
			tb2 = BkdBuf[uv+1][(left[cr_MB_y].pos_y>>1)*iwidth + (left[cr_MB_y].pos_x>>1)];

		if (!left_up_avail || !left_avail[0] || !left_avail[1] || !up_avail)
		  error("unexpected PLANE_8 chroma intra prediction mode",-1);
	    
		ih = cr_MB_x/2*(ta1 - tb1);
		for (i=0;i<cr_MB_x/2-1;i++)
		{
			mbx = (int)((up.pos_x+cr_MB_x/2  +i)/16);
			mby = (int)(up.pos_y/16);			
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				ta2 = imgUV[uv][up.pos_y][up.pos_x+cr_MB_x/2  +i];
			else
				ta2 = BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x+cr_MB_x/2  +i)>>1)];

			mbx = (int)((up.pos_x+cr_MB_x/2-2-i)/16);
			mby = (int)(up.pos_y/16);			
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				ta3 = imgUV[uv][up.pos_y][up.pos_x+cr_MB_x/2-2-i];
			else
				ta3 = BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x+cr_MB_x/2-2-i)>>1)];

		  ih += (i+1)*(ta2 - ta3);
		}
	    
		iv = cr_MB_y/2*(tb2 - tb1);
		for (i=0;i<cr_MB_y/2-1;i++)
		{
			mbx = (int)(left[cr_MB_y/2+1+i].pos_x/16);
			mby = (int)(left[cr_MB_y/2+1+i].pos_y/16);			
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				tb3 = imgUV[uv][left[cr_MB_y/2+1+i].pos_y][left[cr_MB_y/2+1+i].pos_x];
			else
				tb3 = BkdBuf[uv+1][(left[cr_MB_y/2+1+i].pos_y>>1)*iwidth + (left[cr_MB_y/2+1+i].pos_x>>1)];

			mbx = (int)(left[cr_MB_y/2-1-i].pos_x/16);
			mby = (int)(left[cr_MB_y/2-1-i].pos_y/16);			
			if(blk_dec_enb[mby*mb_width+mbx]==1)
				tb4 = imgUV[uv][left[cr_MB_y/2-1-i].pos_y][left[cr_MB_y/2-1-i].pos_x];
			else
				tb4 = BkdBuf[uv+1][(left[cr_MB_y/2-1-i].pos_y>>1)*iwidth + (left[cr_MB_y/2-1-i].pos_x>>1)];
		...
		case HOR_PRED_8:
		  ...
		  for (j=0;j<2;j++)
		  {
			...
			for(i=0;i<2;i++)
			{
			  ...
			  {
				mbx = (int)(left[1+jj+joff].pos_x/16);
				mby = (int)(left[1+jj+joff].pos_y/16);			
				if(blk_dec_enb[mby*mb_width+mbx]==1)
					pred = imgUV[uv][left[1+jj+joff].pos_y][left[1+jj+joff].pos_x];
				else
					pred = BkdBuf[uv+1][(left[1+jj+joff].pos_y>>1)*iwidth + (left[1+jj+joff].pos_x>>1)];
		...
		case VERT_PRED_8:
		  ...
		  for (j=0;j<2;j++)
		  {
			...
			for(i=0;i<2;i++)
			{
			  ...
			  for (ii=0; ii<cr_MB_x/2; ii++)
			  {
				mbx = (int)((up.pos_x+ii+ioff)/16);
				mby = (int)(up.pos_y/16);
				if(blk_dec_enb[mby*mb_width+mbx]==1)
					pred = imgUV[uv][up.pos_y][up.pos_x+ii+ioff];
				else
					pred = BkdBuf[uv+1][(up.pos_y>>1)*iwidth + ((up.pos_x+ii+ioff)>>1)];
				for (jj=0; jj<cr_MB_y/2; jj++)
				  img->mpr[ii+ioff][jj+joff]=pred;
			  }
			}
    ...
    for (j=0;j<2;j++)
    {
      ...
      if(!residue_transform_flag)
      {
        if(!chroma)
        {
		  ...
		  if(extract_flag==1)
		  {
			  res[i][j] =max(0,min(img->max_imgpel_value,(m6[j]+m6[j1]+DQ_ROUND)>>DQ_BITS));
			  res[i][j1]=max(0,min(img->max_imgpel_value,(m6[j]-m6[j1]+DQ_ROUND)>>DQ_BITS));
		  }
        ...
        } else 
        {
		  ...
		  if(extract_flag==1)
		  {
			  res[i][j] =max(0,min(img->max_imgpel_value_uv,(m6[j]+m6[j1]+DQ_ROUND)>>DQ_BITS));
			  res[i][j1]=max(0,min(img->max_imgpel_value_uv,(m6[j]-m6[j1]+DQ_ROUND)>>DQ_BITS));
		  }
    ...
    for (i=0;i<BLOCK_SIZE && lossless_qpprime;i++)
      for (j=0;j<BLOCK_SIZE;j++)
        if(!chroma)
		{
		  // ws 070827
		  if(extract_flag==1)
				res[i][j] = max(0,min(img->max_imgpel_value,img->cof[i0][j0][i][j]));
        ...
        else
		{
		  // ws 070827
		  if(extract_flag==1)
				res[i][j] = max(0,min(img->max_imgpel_value_uv,img->cof[i0][j0][i][j]));
