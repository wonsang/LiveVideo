...
int first_object_tracking_main()
{
  ...
  //Added by Wonsang You at FEB 24, 2007
  img->Transform8x8Mode = 0;
  //Added-End
  ...
  first_partial_decode_one_frame(img, input, snr);
...
{
  while (last_partial_decode_one_frame(img, input, snr) != EOS)
    ;

  //Deleted by Wonsang You at FEB 7, 2007
  //report(input, img, snr);
  //Deleted-End
  free_slice(input,img);
  FmoFinit();
...
void init_conf(struct inp_par *inp, char *config_filename)
{
  ...
  //start ws 070810
  fscanf(fd,"%s",inp->outfile);                //skip an input file
  fscanf(fd,"%*[^\n]");
  //end ws
